...
# %%
# Choosing file to be used
desired_generated_report_file = r"C:\Users\balatarsini_avinitya\Downloads\Topline Report - July\Full Data\MY CPD Male Skincare Topline (Use This) 2025-12-26 Corn (1)_CLEAN_full.xlsx"
desired_generated_report_file1 = r"C:\Users\balatarsini_avinitya\Downloads\Topline Report - July\Full Data\MY CPD Female Skincare (MASS Only) Topline (Use This) 2025-12-26 Corn (1)_CLEAN_full.xlsx"
...
generated_report_path1 = os.path.join(generated_report_folder, desired_generated_report_file1)

# Load the generated data
generated_report = pd.read_excel(generated_report_path, sheet_name=None)
generated_report1 = pd.read_excel(generated_report_path1, sheet_name=None)

# Load the empty template from 'Commentary Template' with multiple sheets
template_path_commentary = os.path.join(commentary_template_folder, "MY Skincare CPD Commentary Template.xlsx")
...
#Female
sheet_data4 = generated_report1['1-Table-1']
sheet_data5 = generated_report1['2-Table-1']
sheet_data6 = generated_report1['3-Table-1']
sheet_data7 = generated_report1['4-Table-1']
...
# %%
cpdsummary_df = sheet_data.iloc[10:47, 0:38]
cpdsummary_df1 = sheet_data2.iloc[10:47, 0:38]
cpdsummary_df7 = sheet_data1.iloc[:,0:38]

# Retrieve and format the month and year
## [:3] takes the 1st 3 char of the str; col 35 in python means col 36 in excel
month_year = cpdsummary_df7.columns[35]
...
sales_value = cpdsummary_df7.iloc[1, 35]
if sales_value >= 1e6:
...
evo_percentage = cpdsummary_df7.iloc[1, 36]
evo_percentage_formatted = f"{evo_percentage:.1f}%"         # -3.2744
...
brands_to_filter = ['Total CPD', 'Garnier', "L'Oreal Paris"]
...
# Value (this month)
## Python treats row 1 in excel as header by default, so row 1 in pandas is row 3 in excel (1 for header, 1 for zero-idx)
...
functionsummary_df = combine.iloc[1:7,0:74]
...
replacements = {'ANTI ACNE': 'Anti Acne',
...
# %%
#this month
function_df_asc = functionsummary_df[~functionsummary_df['Function'].isin(['BASIC', 'HYDRATION','OIL CONTROL'])][['Function', 'Month12 Evo', 'Month12 Unit Evo']].sort_values(by='Month12 Evo', ascending=False)
...
# %%
#Format
formatsummary_df = combine.iloc[7:10,0:74]
...
# %%
#Brand
brandsummary_df = combine.iloc[11:47,0:74]
brandsummary_df.columns = column_names_function

# brand names
brand_names = ['Garnier',"L'Oreal Paris",'Nivea','Biore','Gatsby','NANO WHITE','DASHING','SAFI','BAD LAB',
            'FAIR & HANDSOME','Kahf','OTHER BRANDS','Exclusive brand + Private label']
...
replacements = {'NANO WHITE': 'Nano White',
                'DASHING': 'Dashing',
                'SAFI': 'Safi',
                'BAD LAB': 'Bad Lab',
...
# Check if sheet_data1 has at least 2 rows to avoid IndexError (for row 2)
if len(sheet_data1) > 1:
    # Access the value from `sheet_data1` for row 2, column I (8th column in 0-based indexing)
    value = sheet_data1.iloc[1, 8]  # Use iloc to access row 2 (index 1) and column I (index 8)
    ...
    if pd.notna(value):  # Check if value is not NaN
        ...
        sentence1 = "1. Value in cell 2I is not available."
else:
    sentence1 = "1. Row 2 does not exist in sheet_data1."

print(sentence1)

# Access the values from the required cells in `sheet_data`
growth_value = sheet_data.iloc[10, 8]  # Value from cell 12I (index 11, column index 8)
print(sheet_data.iloc[1:15, 8])

# Calculate the ratio for "ahead/behind" calculation
ratio_ahead_behind = growth_value / rounded_value if rounded_value != 0 else 0.0

# Round the ratio to one decimal place after calculating
ratio_ahead_behind = round(ratio_ahead_behind, 1)
ahead_behind = ahead_or_behind(growth_value, value)

# Calculate the market share difference
market_share_difference = round(sheet_data.iloc[10, 7] - sheet_data.iloc[10, 5], 1)

# Round all values to one decimal place for consistent formatting
market_share_current_rounded = round(sheet_data.iloc[10, 7], 1)  # 12H (index 11, column 7)
rounded_growth_value = round(growth_value, 1)  # Define rounded_growth_value here
...
brands = sheet_data.iloc[12:45, 0]           # Brands from row 14 to row 43 (index 13 to 42)
column_h_values = sheet_data.iloc[12:45, 7]  # Values from column H (index 13 to 42)
column_f_values = sheet_data.iloc[12:45, 5]  # Values from column F (index 13 to 42)

# Initialize variables to keep track of the best result
max_difference = float('-inf')
best_brand = None

# List of specific brands to check
brands_to_check = [
    "Garnier", "L'Oreal Paris", "Nivea", "Biore", "Gatsby", "NANO WHITE", "DASHING",
    "SAFI", "BAD LAB", "FAIR & HANDSOME", "Kahf","OTHER BRANDS", "Exclusive brand + Private label"
]
...
# %% [markdown]
# # FEMALE + MUR (mass only)

# %% [markdown]
# CPD Summary Table

# %%
cpdsummary_df2 = sheet_data4.iloc[19:104, 0:38]
cpdsummary_df3 = sheet_data6.iloc[19:104, 0:38]
cpdsummary_df8 = sheet_data5.iloc[:, 0:38]

# Retrieve and format the month and year
month_year = cpdsummary_df8.columns[35]
month_year_formatted = f"{month_year[:3]}'{month_year[-2:]} vs. {month_year[:3]}'{int(month_year[-2:])-1:02d}"

# Retrieve and format the market sales value
sales_value = cpdsummary_df8.iloc[1, 35]
if sales_value >= 1e6:
    sales_value_formatted = f"{sales_value / 1e6:.1f} mil"
else:
    sales_value_formatted = f"{sales_value / 1e3:.1f} thousand"
...
# Retrieve and format the evolution percentage
evo_percentage = cpdsummary_df8.iloc[1, 36]
...
# list of brands to filter
brands_to_filter1 = ['TOTAL CPD', 'GARNIER', "L'OREAL PARIS"]
...
functionsummary_df1 = combine1.iloc[1:8,0:74]
print(functionsummary_df1)
functionsummary_df1.columns = column_names_function

replacements = {'WHITENING': 'Female Brightening',
                'BASIC': 'Basic',
                'HYDRATION': 'Hydration',
                'ANTI AGE': 'Female Anti-Aging',
                'ANTI ACNE': 'Female Anti-Acne',
                'SENSITIVE' : 'Female Sensitive'}

#Function
# Selecting data for this month
function_df_asc3 = functionsummary_df1[~functionsummary_df1['Function'].isin(['OIL CONTROL', 'BASIC + HYDRATION'])][['Function', 'Month12 Evo', 'Month12 Unit Evo']].sort_values(by='Month12 Evo', ascending=False)
...
# %%
#Format
formatsummary_df1 = combine1.iloc[8:19,0:74]
# formatsummary_df1_1 = combine1.iloc[[21], 0:74]
# formatsummary_df1 = pd.concat([formatsummary_df1, formatsummary_df1_1], axis=0, ignore_index=True)
formatsummary_df1.columns = column_names_function
print(formatsummary_df1.head(5))
...
format_df_asc3 = formatsummary_df1[~formatsummary_df1['Function'].isin(['CREAM/GEL/OIL/LOTION', 'ESSENCE/SERUM', 'COSMETIC / TREATMENT WATER','THERMAL / SPRING WATER','AMPOULE'])][['Function', 'Month12 Evo', 'Month12 Unit Evo']].sort_values(by = 'Month12 Evo', ascending = False)
print(format_df_asc3)
...
# %%
#Brand
garnier = combine1.iloc[[20], 0:74]
loreal = combine1.iloc[[25], 0:74]
brandsummary_df1 = combine1.iloc[31:84,0:74]
others = combine1.iloc[[86], 0:74]
exclusive = combine1.iloc[[87], 0:74]
brandsummary_df1 = pd.concat([garnier, loreal, brandsummary_df1, others, exclusive], axis=0, ignore_index=True)
...
# Check if sheet_data5 has at least 2 rows to avoid IndexError (for row 2)
if len(sheet_data5) > 1:
    # Access the value from `sheet_data5` for row 2, column I (8th column in 0-based indexing)
    value = sheet_data5.iloc[1, 8]  # Use iloc to access row 2 (index 1) and column I (index 8)
    ...
    if value is not None:
        ...
        sentence_1 = "1.Value in cell 2I is not available."
else:
    sentence_1 = "1.Row 2 does not exist in sheet_data5."

print(sentence_1)

# Access the values from the required cells in `sheet_data4`
growth_value = sheet_data4.iloc[19, 8]  # Value from cell 21I (index 20, column index 8)
...
# Calculate the market share difference
market_share_difference = round(sheet_data4.iloc[19, 7] - sheet_data4.iloc[19, 5] , 1)

# Round all values to one decimal place for consistent formatting
market_share_current_rounded = round(sheet_data4.iloc[19, 7], 1)  # Market value in column H
...
# Access the DataFrame for the specified rows and columns
brands = sheet_data4.iloc[20:87, 0]  # Brands from row 22 to row 106 (index 21 to 105)
column_h_values = sheet_data4.iloc[20:87, 7]  # Values from column H (index 21 to 105)
column_f_values = sheet_data4.iloc[20:87, 5]  # Values from column F (index 21 to 105)
...
# List of specific brands to check
brands_to_check = [
    "GARNIER", "L'OREAL PARIS", "MAYBELLINE", "SAFI", "BIO ESSENCE", "HADO LABO", "AIKEN",
    "FAIR & LOVELY", "OLAY", "NANO WHITE", "SKINTIFIC", "NUTOX", "HIMALAYA", "SIMPLE", "CLINELLE", "ST.IVES", "NIVEA", "WARDAH", "TORRIDEN", "Others", "EXCLUSIVE + PRIVATE LABELS"
]
...
load_data_into_template(top_brands_df, wb["Men CPD"], 18, 15)
...
load_data_into_template(sentence0_df, sheet, 27, 2)  # Row 17, Column B (2)
load_data_into_template(sentence1_df, sheet, 28, 2)  # Row 18, Column B (2)
load_data_into_template(growth_sentence_df, sheet, 29, 2)  # Row 19, Column B (2)
load_data_into_template(sentence_df, sheet, 30, 2)  # Row 20, Column B (2)
...
#this month
load_data_into_template(Market_Value3, wb["FEMALE + MUR (M Only)"], 8, 6)
load_data_into_template(Ms_Value3, wb["FEMALE + MUR (M Only)"], 9, 5)
load_data_into_template(Evo_Value3, wb["FEMALE + MUR (M Only)"], 9, 6)
load_data_into_template(Market_Unit3, wb["FEMALE + MUR (M Only)"], 8, 11)
load_data_into_template(Ms_Unit3, wb["FEMALE + MUR (M Only)"], 9, 10)
load_data_into_template(Evo_Unit3, wb["FEMALE + MUR (M Only)"], 9, 11)

#last month
load_data_into_template(Ms_Value4, wb["FEMALE + MUR (M Only)"], 9, 4)
load_data_into_template(Ms_Unit4, wb["FEMALE + MUR (M Only)"], 9, 9)


#last 2 months
load_data_into_template(Ms_Value5, wb["FEMALE + MUR (M Only)"], 9, 3)
load_data_into_template(Ms_Unit5, wb["FEMALE + MUR (M Only)"], 9, 8)

# Market Summary Table
#this month
load_data_into_template(function_df_asc3, wb["FEMALE + MUR (M Only)"], 11, 15)
load_data_into_template(format_df_asc3, wb["FEMALE + MUR (M Only)"], 17, 15)
load_data_into_template(top_brands_df3, wb["FEMALE + MUR (M Only)"], 22, 15)

# Place the final_sentence in the 2nd row and 2nd column using the load_data_into_template function
load_data_into_template(final_sentence_df2, wb["FEMALE + MUR (M Only)"], 2, 2)

# Adding month names to the specified cells directly
sheet = wb["FEMALE + MUR (M Only)"]
sheet.cell(row=7, column=5, value=current_month_name)
sheet.cell(row=7, column=4, value=month_1_before_name)
sheet.cell(row=7, column=3, value=month_2_before_name)
sheet.cell(row=7, column=10, value=current_month_name)
...
growth_sentence_1_df = pd.DataFrame([[growth_sentence1]])
sentence_2_df = pd.DataFrame([[sentence_2]])

# Use the load_data_into_template function to insert sentences into the correct cells
load_data_into_template(sentence_0_df, sheet, 27, 2)  # Row 17, Column B (2)
load_data_into_template(sentence_1_df, sheet, 28, 2)  # Row 18, Column B (2)
load_data_into_template(growth_sentence_1_df, sheet, 29, 2)  # Row 19, Column B (2)
load_data_into_template(sentence_2_df, sheet, 30, 2)  # Row 20, Column B (2)
...
print(f"saved to {output_template_path}")
